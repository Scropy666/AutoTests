...
namespace AutoTests
{
    public class DNSTests
    {
        ...
        public void TestPriceFilter()
        {
            ...
            driver.FindElement(By.XPath("//input[contains(@placeholder , 'от ')]")).Click();
            new WebDriverWait(driver, TimeSpan.FromSeconds(5)).Until(x => driver.FindElement(By.XPath("//input[contains(@placeholder , 'от ')]")));
            driver.FindElement(By.XPath("//input[contains(@placeholder , 'от ')]")).Clear();
            new WebDriverWait(driver, TimeSpan.FromSeconds(5)).Until(x => driver.FindElement(By.XPath("//input[contains(@placeholder , 'от ')]")));
            driver.FindElement(By.XPath("//input[contains(@placeholder , 'от ')]")).SendKeys("30000");

            new WebDriverWait(driver, TimeSpan.FromSeconds(5)).Until(x => driver.FindElement(By.XPath("//input[contains(@placeholder , 'до ')]")));
            driver.FindElement(By.XPath("//input[contains(@placeholder , 'до ')]")).Clear();
            new WebDriverWait(driver, TimeSpan.FromSeconds(5)).Until(x => driver.FindElement(By.XPath("//input[contains(@placeholder , 'до ')]")));
            driver.FindElement(By.XPath("//input[contains(@placeholder , 'до ')]")).SendKeys("50000");
            ...
            actualPrices.ToList().ForEach(price => Assert.IsTrue(price >= 30000 && price <= 50000, "The price is not included in the range"));
        ...
        public void TestTooltipText()
        {
            ...
            // из-за того, что первый pagedown не доходит до конца страницы, используем предыдщую команду еще один раз
            driver.FindElement(By.CssSelector("body")).SendKeys(Keys.PageDown);
        ...
        public void TestInputInvalidPhoneNumber()
        {
            ...
            "Error 'Заполните поле' is enabled when phone number input is incorrect");
        }
